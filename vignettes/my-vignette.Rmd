...
To run `bamtobed`, only the path to the BAM file(s) is required, possibly coupled with the location of the output directory. For convenience, it is suggested to to rename the BAM files before their acquisition, in order to maintain the same nomenclature of the samples through the whole process (however, it is possible to modify them while running `bedtolist`, as explained below).
  ...
  The resulting files will contain, for each read, the name of the transcript on which it alignes, the position of its first and last nucleotide, its length and the associated strand. These informaton can be exploited for several downstreaming analyes such as generate the meta profiles of the extremities of the reads and look at the distribution of their length (different lengths of the reads may be associated to different conformation of ribosomes), as desribed below. The BED files will appear like the following (the header is here added for clarity):
  ...
  |  ENSMUST00000000001.4  |  142  |  170  |  29  |  +  |
   ...
   An annotation file (see next chapter for more information) is also required.
