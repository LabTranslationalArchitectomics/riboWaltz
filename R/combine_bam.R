...
#' @param rm_version Logical value whether to remove the transcript version at
...
bamtolist <- function(bamfolder, annotation, transcript_align = TRUE, 
                      name_samples = NULL, rm_version = FALSE,
                      granges = FALSE) {
  ...
  for (n in names) {
    ...
    if(nreads != nrow(dt)){
      if(nrow(dt) == 0){
        stop(sprintf("%s M  (%s %%) reads removed: reference transcript ID not found in the annotation table\n\n",
                     format(round((nreads - nrow(dt)) / 1000000, 2), nsmall = 2), 
                     format(round(((nreads - nrow(dt)) / nreads) * 100, 2), nsmall = 2) ))
      } else{
        cat(sprintf("%s M  (%s %%) reads removed: reference transcript ID not found in the annotation table\n", 
                    format(round((nreads - nrow(dt)) / 1000000, 2), nsmall = 2), 
                    format(round(((nreads - nrow(dt)) / nreads) * 100, 2), nsmall = 2) )) 
      }
    }
...
#' sequence; iii) its length; iv) the strand on which it aligns. Please note:
#' this function relies on the \emph{bamtobed} utility of the BEDTools suite and
#' can be only run on UNIX, LINUX and Apple OS X operating systems. Moreover, to
#' generate R data structures containing reads information, the
#' \code{\link{bedtolist}} must be run on the resulting BED files. For these
#' reasons the authors suggest the use of \code{\link{bamtolist}}.
#'
#' @param bamfolder Character string specifying the path to the folder storing
...
#' @param annotation Data table as generated by \code{\link{create_annotation}}.
#'   Please make sure the name of reference transcripts in the annotation data
#'   table match those in the BED files (see also \code{rm_version}).
...
#' @param rm_version Logical value whether to remove the transcript version at
#'   the end of their ID, usually dot-separated. It might be required to make
#'   the transcripts IDs in the BED files match those in the annotation table.
#'   Default is FALSE.
#' @param granges Logical value whether to return a GRangesList object. Default
#'   is FALSE i.e. a list of data tables is returned instead (the required input
#'   for \code{\link{length_filter}}, \code{\link{psite}},
