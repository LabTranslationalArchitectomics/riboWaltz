...
#'   case, each member of the list should include the name of the replicate(s)
...
#' @param mirrored Logical value whether to show mirrored metaprofiles when
...
metaprofile_psite <- function(data, annotation, sample, multisamples = "separated",
                              mirrored = TRUE, scale_factors = NULL,
  ...
  if(is.list(sample) & length(sample) > 2 & mirrored == TRUE) {
    cat("\n")
    warning("parameter sample is a list of dimension > 2. Parameter mirrored won't be considered\n", call. = FALSE)
    mirrored <- FALSE
  }
  ...
  if(length(sample_l) == 1){
    colour <- colour[1]
    names(colour) <- "sample_name"
  } else{
    if(length(sample_l) == length(colour)) {
      names(colour) <- names(sample_l)
    } else {
      cat("\n")
      warning("wrong number of specified colours. Colours will be set automatically\n", call. = FALSE)
      if(length(sample_l) == 2) {
  ...
  if(multisamples != "separated"){
    ...
    if(mirrored == TRUE & length(col_plot_sel) == 2){
    ...
    melt_plot_dt <- melt.data.table(plot_dt[, ..col_sel],
                                    id.vars = c("distance", "reg"), variable.name = "sample"
    )[, sample := factor(sample, levels = as.character(unlist(col_plot_sel)))]
    
    plot <- ggplot(melt_plot_dt, aes(x = distance, y = value, color = sample)) +
      geom_line(size = 1.50)
    
    if(length(colour) != 0){
      plot <- plot + scale_color_manual(labels = names(sample_l),values = as.character(colour))
    }
    ...
    if(multisamples == "average"){
      for(col_plot in names(col_plot_sel)){
        if(length(col_plot_se_sel[[col_plot]]) != 0){
          ...
          plot <- plot + geom_ribbon(data = plot_dt, aes_string(x = "distance",
                                                                ymin = paste0("minus_se_", col_plot),
                                                                ymax = paste0("plus_se_", col_plot)),
                                     color = NA, fill = colour[col_plot], alpha = 0.20, inherit.aes = FALSE)
        } else {
          plot <- plot
        }
      }
...
#' sample_list <- list("Only_28" = c("subsample_28nt"), "All" = c("whole_sample"))
#' metaheat_comparison <- metaheatmap_psite(metaheat_df, mm81cdna, sample = sample_list)
#' @import data.table
#' @import ggplot2
#' @export
