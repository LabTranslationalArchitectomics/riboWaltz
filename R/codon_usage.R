...
#' 
#' For a specified sample computes the codon usage i.e. how many in-frame P-sites along
#' the coding sequence are associated to the 64 triplets, normalized for the
#' frequency of the triplets. This function also allows to compare the codon
#' usage of the selected sample with a set of values provided by the user.
#' 
...
#'   \code{\link{psite_info}} specifying the \code{sequence_folder} option. If
#'   the data frame of interest does not include a \emph{psite_codon} column,
#'   the information about the nucleotide sequnce of the P-sites will be
#'   automatically generated throughout the function.
...
#'   transcriptome. This argument is used to extract for each read the three
#'   nucletotides corresponding to the identified P-sites. Coupled with the
#'   annotation file it is also required to compute the frequency of the
#'   triplets along the coding sequences and normalize the codon usage values.
...
#'   interest. The data frame must contain at lease the 64 codons and the
...
#'   (named ""df) with the associated data. Additional ggplot2 object
...
#' 
#' 
codon_usage_psite <- function(data, annotation, sample, sequence_folder, codon_usage = NULL, transcripts = NULL, label = NULL) {
  ...
  norm.table$class <- factor(norm.table$class, levels=c("start", "cds", "stop"), labels=c("Start codo", "cds", "Stop codon"))
  ...
  bp <- ggplot(norm.table,aes(x = codon, y = usage_index, fill = class)) +
    ...
    scale_fill_manual(name = "", breaks=c("Start codo","Stop codon"), values = c("#104ec1", "gray60", "darkred")) +
  ...
  if(length(codon_usage) != 0){
    ...
    pcomp <- ggplot(norm.table,aes(x = usage_index, y = comp_values, colour = class)) +
      geom_point(alpha = 0.9, size=bs*0.08) +
      ggrepel::geom_text_repel(aes(usage_index, comp_values, label = codon), show.legend = F) +
      scale_colour_manual(name = "", breaks=c("Start codo","Stop codon"), values = c("#104ec1", "gray60", "darkred")) +
      theme_bw(base_size = bs) +
      theme(legend.position = "top") +
      scale_x_continuous("Codon usage index", limits=c(0,1), breaks=c(0,0.5,1)) +
      scale_y_continuous("Codon usage index match", limits=c(0,1), breaks=c(0,0.5,1))
