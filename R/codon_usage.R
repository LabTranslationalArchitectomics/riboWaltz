...
#' @param codon_values A data frame containing codon-specific values provided by
#'   the user. These values are compared with the empirical codon usage indexes
#'   of the sample of interest. The data frame must contain at least the 64
...
#'   frame is also returned by \code{\link{codon_usage_psite}} itself. Default
#'   is NULL.
...
codon_usage_psite <- function(data, annotation, sample, fastapath = NULL, fasta_genome = TRUE,
  ...
  if(length(codon_values) != 0){
    ...
    if (nrow(codon_values) < 64 | length(unique(rownames(codon_values))) < 64){
      cat("\n")
      stop("\nERROR: number of different triplets in codon_values < 64. At least one codon is missing\n\n")
    } else {
      if (length(unique(rownames(codon_values))) > 64){
        cat("\n")
        stop("\nERROR: number of different triplets in codon_values > 64. Too many codons\n\n")
      }
    }
  }
  
  if(("psite_codon" %in% colnames(data)) & 
     (length(fastapath) != 0 | length(bsgenome) != 0 | length(gtfpath) != 0 | length(txdb) != 0)){
    warning("column psite_codon already present.\nAdditional nucleotide sequences and annotation files will be ignored\n")
  } else {
  ...
  cu_level <- apply(expand.grid(one = c("A","C","G","U"), two = c("A","C","G","U"), three=c("A","C","G","U")), 1,
  ...
  norm.table$value <- norm.table$count / seq_freq[as.character(norm.table$codon)]
  norm.table$value <- norm.table$value - min(norm.table$value)
  norm.table$value <- norm.table$value / max(norm.table$value)
  norm.table <- norm.table[order(norm.table$value),]
  ...
  bs <- 30
  bp <- ggplot(norm.table,aes(x = codon, y = value, fill = class)) +
  ...
  output <- list()
  output[["plot"]] <- bp
  output[["df"]] <- norm.table[, c("codon", "class", "value",  "aa")]

  if(length(codon_values) != 0){
    codon_values$codon <- gsub("T", "U", cu$codon)
    norm.table$comp_values <- codon_values[as.character(norm.table$codon), "value"]
    ...
    correlation <- round(cor(norm.table$value, norm.table$comp_values), 3)
    
    bs <- 30
    pcomp <- ggplot(norm.table,aes(x = value, y = comp_values, colour = class)) +
